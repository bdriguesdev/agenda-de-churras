...
import { editChurrasco } from '../../../actions/churrasco';
...
const mapDispatchToProps = dispatch => {
    return {
        editChurrasco: (churrascoInfo, token) => dispatch(editChurrasco(churrascoInfo, token))
    };
};

const EditModal = props => {
    const { bind:titleBind } = useInput(props.churrasco.title);
    const { bind:descriptionBind } = useInput(props.churrasco.description);
    const { bind:dateBind } = useInput(props.churrasco.date.split("T")[0]);
    const { bind:foodPriceBind } = useInput(props.churrasco.foodPrice);
    const { bind:foodAndDrinkPriceBind } = useInput(props.churrasco.foodAndDrinkPrice);

    const handleEditChurrasco = () => {
        if(!props.loading) {
            props.editChurrasco(
                {
                    ...
                    churrascoId: props.churrasco._id
                },
                props.token
            );
        }
    ...
    return (
        <FormModal 
            ...
            path=".churrasco .form__container"
            submit={handleEditChurrasco}
